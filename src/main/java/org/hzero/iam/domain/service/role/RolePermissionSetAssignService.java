...
public class RolePermissionSetAssignService {
    ...
    public void assignPermissionSetsToTenantManager(Long tenantId, Long permissionSetId, RolePermissionType rolePermissionType) {
        ...
        //查询租户管理员模板角色
        Role tenantRoleTpl = roleRepository.selectOneRoleByCode(Constants.ORGANIZATION_TENANT_ROLE_TPL_CODE);
        if (tenantRoleTpl == null) {
        ...
        queryParam.setInheritRoleId(tenantRoleTpl.getId());
        queryParam.setTenantId(tenantId);
        Role tenantRole = roleRepository.selectOne(queryParam);
        if (tenantRole == null) {
        ...
        //直接挂上去
        RolePermission rolePermission = new RolePermission(tenantRole.getId(), permissionSetId, YesNoFlag.NO,
                YesNoFlag.YES, rolePermissionType.name(), tenantRole.getTenantId());

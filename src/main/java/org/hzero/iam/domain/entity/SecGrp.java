...
public class SecGrp extends AuditDomain {
    ...
    public void setupCurrentRole(Long roleId) {
        ...
        if (siteLevel) {
            this.level = HiamResourceLevel.SITE.value();
        } else if (tenantLevel) {
            this.level = HiamResourceLevel.ORGANIZATION.value();
        }
    }
    ...
    @NotBlank
    @LovValue(lovCode = "HIAM.SECURITY_GROUP_LEVEL", meaningField = "levelMeaning")
